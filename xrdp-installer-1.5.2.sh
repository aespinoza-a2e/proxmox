#!/bin/bash
#####################################################################################################
# Script_Name : xrdp-installer-1.5.2.sh
...
# Version : 1.5.2
# History : 1.5.2 - Change Code to Check OS
...
/bin/echo -e "\e[1;36m   !   Custom version for A2e Technologies: aespinoza                !\e[0m"
...
array=("Ubuntu 20.04" "Ubuntu 22.04" "Ubuntu 24.04" "Ubuntu 24.10" "Pop!_OS 20.04" "Pop!_OS 22.04" "Debian" "Mint" "LMDE" "Zorin")
...
if [[ *"$version"* = *"Debian"*  ]]
then
   CustomPix="logo_xrdpd.bmp"
else 
   CustomPix="logo_xrdp.bmp"
fi
...
then
   ...
   wget https://raw.githubusercontent.com/aespinoza-a2e/proxmox/refs/heads/develop/"$CustomPix"
fi
...
if [[ $(stat -c '%a' /usr/local/share/xrdp/logo_xrdp.bmp) != '644' ]]
then 
    /bin/echo -e "\e[1;32m        |-| Fixing permissions griffon_logol_xrdp.bmp....: Proceeding"
    sudo chmod 644 /usr/local/share/xrdp/logo_xrdp.bmp
else 
...
/bin/echo -e "\e[1;36m   |                                                                |\e[0m"
/bin/echo -e "\e[1;36m   | Credits : Written by Griffon - June   2024                     |\e[0m"
